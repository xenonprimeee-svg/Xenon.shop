...
<head>
  ...
  <title>Xylo Hacks | Notifications</title>
  <script src="https://cdn.tailwindcss.com"></script>
  <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.3/font/bootstrap-icons.min.css">
  <link rel="stylesheet" href="/assets/css/vip-purple.css?v=1773418345">
  <style>
    body{background:#070a12;color:#e5e7eb;min-height:100vh}
    .glass-card{position:relative;overflow:hidden;border:1px solid rgba(168,85,247,.16);background:rgba(10,14,28,.58);backdrop-filter:blur(14px);-webkit-backdrop-filter:blur(14px);box-shadow:0 18px 54px rgba(0,0,0,.62),0 0 0 1px rgba(168,85,247,.18) inset,0 0 26px rgba(168,85,247,.12)}
    .glass-card::before{content:"";position:absolute;inset:-1px;background:radial-gradient(340px 180px at 24% 18%, rgba(168,85,247,.30), transparent 60%),radial-gradient(260px 180px at 80% 26%, rgba(99,102,241,.22), transparent 62%);opacity:.85;pointer-events:none}
    .glass-card>*{position:relative;z-index:1}
    .cta{background:linear-gradient(135deg, rgba(168,85,247,.95), rgba(99,102,241,.90));color:#fff;border:1px solid rgba(255,255,255,.18);box-shadow:0 16px 44px rgba(168,85,247,.30),0 0 0 1px rgba(168,85,247,.22) inset,0 0 26px rgba(99,102,241,.22)}
  </style>
</head>
...
<div class="flex items-center justify-between sm:justify-end space-x-3 w-auto">
  ...
  <div class="xv-centerNav" aria-label="Primary navigation">
      <div class="xv-tabs">
        ...
        <a href="/notifications.php" class="xv-tab xv-tabActive" aria-label="Notifications">
...
(function markActiveNav(){
  ...
  <main class="flex-1 overflow-y-auto p-4 md:p-6 space-y-4 md:space-y-6">
    <section class="glass-card rounded-[26px] p-5 md:p-8">
      <div class="inline-flex items-center gap-2 rounded-full border border-emerald-400/35 bg-emerald-500/10 px-4 py-2 text-sm font-semibold text-emerald-200 mb-5">
        <span class="inline-block h-2.5 w-2.5 rounded-full bg-emerald-400 shadow-[0_0_16px_rgba(16,185,129,.55)]"></span>
        Stay in the loop      </div>
      <h1 class="text-3xl md:text-4xl font-extrabold text-white mb-3">Notifications</h1>
      <p class="text-white/70 text-base md:text-lg max-w-3xl mb-6">Keep up with important store announcements, restock alerts, and customer-facing updates from Xylo Store.</p>
      <div class="flex flex-wrap gap-3">
        <a href="/login.php" class="cta rounded-2xl px-5 py-3 font-semibold text-decoration-none">Login</a>
        <a href="/register.php" class="rounded-2xl px-5 py-3 font-semibold text-white border border-white/15 bg-white/5 hover:bg-white/10 text-decoration-none">Register</a>
      </div>
    </section>
        <section class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
            <article class="glass-card rounded-3xl p-5">
        <div class="flex items-start gap-3">
          <i class="bi bi-bell text-xl text-violet-300"></i>
          <div>
            <h2 class="text-lg font-semibold text-white mb-1">Store updates</h2>
            <p class="text-white/65">Check the latest announcements, notices, and important updates shared by Xylo Store.</p>
          </div>
        </div>
      </article>
            <article class="glass-card rounded-3xl p-5">
        <div class="flex items-start gap-3">
          <i class="bi bi-megaphone text-xl text-violet-300"></i>
          <div>
            <h2 class="text-lg font-semibold text-white mb-1">Never miss an alert</h2>
            <p class="text-white/65">Stay informed about new releases, service notices, and updates that matter to your next purchase.</p>
          </div>
        </div>
      </article>
          </section>
