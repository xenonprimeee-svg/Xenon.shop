...
<head>
  ...
  <style>
    ...
    .xv-showcase-viewport{
      ...
      box-shadow: inset 0 0 0 1px rgba(168,85,247,.14), 0 18px 55px rgba(0,0,0,.55);
      overflow: hidden;
    }
    .xv-showcase-arrow{
      position:absolute; top:50%; transform:translateY(-50%); width:44px; height:44px; border-radius:999px;
      display:flex; align-items:center; justify-content:center;
      background: radial-gradient(circle at 30% 22%, rgba(212,120,255,0.40), transparent 60%), linear-gradient(135deg, rgba(12,7,22,0.78), rgba(132,58,220,0.72)) !important;
      border:1px solid rgba(212,120,255,.30) !important; color: rgba(255,255,255,0.96) !important;
      text-shadow: 0 0 10px rgba(212,120,255,0.55) !important;
      box-shadow: 0 0 0 1px rgba(212,120,255,0.10) inset, 0 12px 26px rgba(0,0,0,.34), 0 0 34px rgba(212,120,255,.20) !important;
      backdrop-filter: blur(10px); -webkit-backdrop-filter: blur(10px); cursor:pointer; z-index:2; opacity:.92; user-select:none;
    }
    .xv-showcase-prev{ left: 14px; }
    .xv-showcase-next{ right: 14px; }
    .xv-showcase-arrow:active{ transform: translateY(-50%) scale(.96); filter: brightness(1.06); }
    .xv-carousel-track{ will-change: transform; display:flex; align-items:center; gap:26px; padding:0 26px; }
    .xv-slide{ height:100%; display:flex; align-items:center; justify-content:center; flex:0 0 auto; opacity:.65; transform:scale(.92); transition:transform .22s ease, opacity .22s ease; }
    .xv-slide.is-active{ opacity:1; transform:scale(1.02); }
    .xv-slide.is-prev, .xv-slide.is-next{ opacity:.85; transform:scale(.96); }
    .xv-slide-inner{ width:var(--xvPhoneW, 220px); height:var(--xvPhoneH, 390px); border-radius:22px; background:rgba(0,0,0,.35); border:1px solid rgba(255,255,255,.10); box-shadow:0 16px 40px rgba(0,0,0,.55); overflow:hidden; }
    .xv-slide img{ display:block; width:100%; height:100%; object-fit:contain; }
    @media (max-width: 640px){ .xv-showcase-arrow{ width:54px; height:54px; font-size:26px; } .xv-showcase-prev{ left:10px; } .xv-showcase-next{ right:10px; } }
      </style>
</head>
  ...
  <div class="max-w-4xl mx-auto">
    <div class="glass xv-purple-panel rounded-2xl p-5">
      <div class="flex items-start justify-between gap-3">
        <div>
          <h1 class="text-3xl md:text-4xl font-extrabold tracking-tight">Reseller Program</h1>
          <p class="text-white/75 mt-2 text-sm md:text-base">Apply to unlock discounted pricing, priority support, and reseller-only access.</p>
                      <div class="mt-3 flex flex-wrap items-center gap-2">
                              <div class="inline-flex items-center gap-2 rounded-full border border-emerald-400/25 bg-emerald-500/10 px-3 py-1.5 text-xs md:text-sm font-bold text-emerald-200 shadow-[0_0_0_1px_rgba(16,185,129,0.08),0_10px_30px_rgba(0,0,0,0.18)]">
                  <span class="opacity-80">Minimum starting budget</span>
                  <span class="text-emerald-100">₱2000 / $40</span>
                </div>
                                            <div class="inline-flex items-center gap-2 rounded-full border border-emerald-400/25 bg-emerald-500/10 px-3 py-1.5 text-xs md:text-sm font-bold text-emerald-200 shadow-[0_0_0_1px_rgba(34,211,238,0.08),0_10px_30px_rgba(0,0,0,0.18)]">
                  <span class="opacity-80">Minimum refill</span>
                  <span class="text-emerald-100">₱1000 / $20</span>
                </div>
                          </div>
                  </div>
    ...
    <section class="mt-6">
      <div class="flex items-center justify-between">
        <h2 class="text-xl font-extrabold tracking-tight">Benefits</h2>
      </div>
      <div class="mt-3 grid gap-3 md:grid-cols-2">
        <div class="glass xv-purple-panel rounded-2xl p-5">
          <div class="font-bold text-white/90">Benefits</div>
          <ul class="mt-3 space-y-2 text-white/70 text-sm list-disc pl-5">
                          <li>Discounted reseller pricing across eligible products</li>
                          <li>Priority updates and faster support response</li>
                          <li>Access to reseller-only announcements and offers</li>
                      </ul>
        </div>
        <div class="glass xv-purple-panel rounded-2xl p-5">
          <div class="font-bold text-white/90">Requirements</div>
          <ul class="mt-3 space-y-2 text-white/70 text-sm list-disc pl-5">
                          <li>Active Telegram account for coordination</li>
                          <li>Good track record and compliance with store rules</li>
                          <li>Willingness to follow posting, support, and delivery policies</li>
                      </ul>
        </div>
      </div>
        ...
        <div class="relative overflow-hidden rounded-2xl border border-purple-500/15 xv-showcase-viewport" data-xv-viewport>
          <div class="xv-carousel-track flex transition-transform duration-300 ease-out" data-xv-track>
                ...
                <div class="xv-slide-inner">
                  <img src="/assets/uploads/reseller_carousel/1000247508_20260505_214400_37fa2a6f.webp" alt="Reseller highlight" loading="lazy" />
                </div>
              </div>
                          <div class="xv-slide shrink-0">
                <div class="xv-slide-inner">
                  <img src="/assets/uploads/reseller_carousel/1000247509_20260505_214400_ba6fb114.webp" alt="Reseller highlight" loading="lazy" />
                </div>
              </div>
                          <div class="xv-slide shrink-0">
                <div class="xv-slide-inner">
                  <img src="/assets/uploads/reseller_carousel/1000247510_20260505_214401_4a938458.webp" alt="Reseller highlight" loading="lazy" />
                </div>
              </div>
                      </div>
    ...
    <section class="mt-6">
      <div class="flex items-center justify-between">
        <h2 class="text-xl font-extrabold tracking-tight">Highlights</h2>
      </div>
      <div class="mt-3 glass xv-purple-panel rounded-2xl p-5">
        <div class="text-white/80 text-sm">What you can expect after approval:</div>
        <ul class="mt-3 space-y-2 text-white/70 text-sm list-disc pl-5">
                      <li>Dedicated reseller access and discounted purchase flow</li>
                      <li>Closer coordination for announcements, updates, and support</li>
                      <li>Better flexibility when managing customers and recurring purchases</li>
                  </ul>
      </div>
    </section>

    <section id="apply" class="mt-6 mb-8">
      <div class="flex items-center justify-between">
      ...
      <div class="mt-3 glass xv-purple-panel rounded-2xl p-5">
        <p class="text-white/70 text-sm">Complete the form below. Our team will review your application and contact you on Telegram if approved.</p>

                            <form method="post" class="mt-4 grid gap-3" data-guest-auth="submit reseller application">
            ...
            <textarea name="reason" rows="4" placeholder="Why do you want to be a reseller?" class="w-full rounded-xl bg-black/30 border border-white/10 px-4 py-3"></textarea>
          ...
          <p class="mt-3 text-xs text-white/50">Submitting sends your details to the admin team for review.</p>
              </div>
    </section>
  </div>
</main>
...
(function(){
  ...
  function initCarousel(root){
    ...
    var idx = 0, gap = 18, step = 0, maxIdx = 0, baseOffset = 0;
    function compute(){
      ...
      step = phoneW + gap;
      baseOffset = Math.floor((vw - phoneW) / 2);
      track.style.gap = gap + 'px';
      track.style.paddingLeft = '0px';
      track.style.paddingRight = '0px';
      slides.forEach(function(s){ s.style.width = phoneW + 'px'; s.style.height = '100%'; });
      if (dotsWrap){
        dotsWrap.innerHTML = '';
    ...
    function paint(){
      ...
      slides.forEach(function(s, i){
        var d = Math.abs(i - idx), inner = s.querySelector('.xv-slide-inner');
        if (!inner) return;
        inner.style.transition = 'transform 180ms ease, opacity 180ms ease, filter 180ms ease';
        if (d === 0){ inner.style.opacity = '1'; inner.style.transform = 'scale(1)'; inner.style.filter = 'saturate(1.06)'; }
        else if (d === 1){ inner.style.opacity = '0.78'; inner.style.transform = 'scale(0.96)'; inner.style.filter = 'saturate(0.92)'; }
        else { inner.style.opacity = '0.55'; inner.style.transform = 'scale(0.92)'; inner.style.filter = 'saturate(0.85)'; }
      ...
      if (dotsWrap){
        Array.prototype.forEach.call(dotsWrap.children, function(dot, i){
          dot.style.background = i === idx ? 'rgba(212,120,255,.95)' : 'rgba(255,255,255,.25)';
          dot.style.boxShadow = i === idx ? '0 0 0 3px rgba(212,120,255,.18)' : 'none';
        });
      }
    }
    function go(n){ idx = clamp(n, 0, maxIdx); paint(); }
    if (btnPrev) btnPrev.addEventListener('click', function(){ go(idx - 1); });
    if (btnNext) btnNext.addEventListener('click', function(){ go(idx + 1); });
    if (dotsWrap) dotsWrap.addEventListener('click', function(e){ var b = e.target.closest('[data-xv-dot]'); if (b) go(parseInt(b.getAttribute('data-xv-dot') || '0', 10)); });
